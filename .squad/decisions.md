...
## Fluent UI System Icons Integration (McManus, 2026-04-07)

**Status:** Applied | **PR:** #103 | **Issue:** #99

### Context

FollowCursor used emoji characters (⏺, ▶, ⏸, 💾, 🔍, ✂, 🎙, 🎬, etc.) for all UI icons. This caused:
- Inconsistent rendering across Windows versions (emoji fonts differ)
- Unprofessional appearance compared to native Windows 11 apps
- Poor alignment with text in buttons and menus
- No theme integration (emoji colors can't be controlled)

### Decision

Replace all emoji icons with SVG assets from **Microsoft Fluent UI System Icons** library.

### Implementation Approach

**Icon Loader Module (`icon_loader.py`):**
- Central `load_icon(name, size, variant, color)` function returns QIcon
- SVG files stored in `followcursor/app/icons/` directory
- Global cache to avoid reloading same icon multiple times
- SVG fill colors replaced dynamically using regex (accepts token references)
- Graceful degradation: returns empty QIcon if SVG file missing

**Icon Assets (20px):**
- Downloaded 17 Fluent UI System Icons from https://github.com/microsoft/fluentui-system-icons
- Variants: Regular (outlined) for most icons, Filled (active/selected states)
- Icons: play, pause, record, save (both regular + filled); search, cut, mic, video, arrow_upload, delete, add, folder_open (regular only)

**Color Integration:**
- `T.BRAND` for primary actions (play, record buttons)
- `T.FG_PRIMARY` for neutral actions (menu items)
- `T.DANGER` for destructive actions (delete)
- All colors from `tokens.py` for theme consistency
...
# After (icon):
btn = QPushButton("  Record")
btn.setIcon(load_icon("record", variant="filled", color=T.BRAND))
```

### Alternatives Considered

1. **Icon Font** — rejected (can't use QIcon API, rendering quality issues)
2. **PNG Rasterized Icons** — rejected (needs multiple size/DPI variants, can't change color at runtime)
3. **QPainter-Drawn Icons** — rejected (high maintenance, inconsistent with Windows 11 visual language)

### Benefits

- Consistent rendering across all Windows versions (SVG is resolution-independent)
- Professional appearance aligned with Windows 11 Fluent 2 design
- Theme integration via token-based coloring
- Performance via caching (icons loaded once, reused)
- Maintainability — adding new icons is just downloading SVG + calling `load_icon()`

### Trade-offs

- Bundle size: 17 SVG files add ~15 KB (negligible)
- Startup time: Icon loading is lazy (only on first use)
- Complexity: `icon_loader.py` adds ~150 lines, but eliminates emoji rendering issues

### Team Guidelines

When adding a new icon:
1. Find the icon at https://github.com/microsoft/fluentui-system-icons/tree/main/assets
2. Download the 20px SVG (regular or filled) to `followcursor/app/icons/`
3. Use `load_icon(name, variant, color)` in your widget
4. Reference a token for color (never hardcode hex)
5. Update this decision doc with the new icon name

Icon naming convention: `{name}_{size}_{variant}.svg` (e.g., `play_20_filled.svg`)

---
*Applied as of 2026-04-07T04:00:00Z*
