...
### 2025-07-18 — Fix: Window thumbnail resolution
**Branch:** `fix/window-thumbnail-resolution`  
**PR:** https://github.com/sabbour/followcursor/pull/109

Window thumbnails in the source picker were blurry because `capture_window_thumbnail()` created GDI bitmaps at only 400×220. PrintWindow renders at whatever bitmap size it's given, so large windows (2560×1440) got squished to ~400×225 — losing all detail.

**Changes:**
- Bumped default `max_w` from 400→800, `max_h` from 220→450 in `capture_window_thumbnail()`
- Updated docstring to explain the 2× density rationale
- MSS fallback inherits new defaults via passed parameters — no separate change needed

**Testing:** All 375 tests pass

## Learnings

### GDI Bitmap Size = Capture Resolution
PrintWindow renders at the exact bitmap dimensions it's given — it doesn't capture at native resolution and then scale. So if you allocate a 400×220 bitmap for a 2560×1440 window, you get 400×220 pixels of detail, period. The lesson: for GDI-based thumbnails, always size the bitmap to your desired output quality, not just display size. 2× the display area gives sharp results with SmoothTransformation downscaling on the display side.
